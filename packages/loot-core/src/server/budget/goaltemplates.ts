...
async function processTemplate(
  ...
  for (let c = 0; c < categories.length; c++) {
    ...
    if (budgeted) {
      if (!force && !category.payDistributeTemplateActive) { //Pay Distribution will ignore the already budgeted rule
        ...
        if (!skipSchedule) {
          ...
          if (template_lines.length > 0) {
            ...
            if (to_budget != null) {
              ...
              budgetAvailable -= to_budget; //May need to change this to set_budget for pay distribution
            }
            if (applyErrors != null) {
              errors = errors.concat(
...
async function applyCategoryTemplate(
  ...
  for (let l = 0; l < template_lines.length; l++) {
    ...
    switch (template.type) {
      ...
      case 'remainder': {
        ...
        //Pay distribution, set_budget sets budgeted while to_budget sets the goal.
        set_budget = goalsReturn.to_budget; //Pay Distribution
        to_budget = goalsReturn.to_budget;
        break;
      }
      case 'average': {
  ...
  if (limitCheck) {
    if (hold && (balance > limit || (payDistributeTemplateActive && balance + category.budgeted > limit))) {
